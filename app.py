...
# --- دالة تنقية الشيتات (لإلغاء Sheet1) ---
...
# ---------------------------------------------------------
# TAB 1: CAPACITY PLANNING (العودة للأصل)
# ---------------------------------------------------------
with tab1:
    with st.sidebar:
        ...
        st.info(f"📅 Days: {working_days} | ⏳ Hours: {base_hours}")
        
        main_file = st.file_uploader("Upload Data.xlsx", type=["xlsx"])

    if main_file:
        sel_lang = st.selectbox("Select Language (Capacity)", get_clean_sheets(main_file))
...
# ---------------------------------------------------------
# TAB 2: INTRADAY (حل مشكلة الجدول الفاضي)
# ---------------------------------------------------------
with tab2:
    ...
    if intra_file:
        ...
        # تنظيف الوقت (Intervals) لضمان عدم ظهور Empty
        ...
        # تحويل لأرقام صحيحة وإلغاء الكسور
        final_intra = df_intra.set_index('Intervals').apply(pd.to_numeric, errors='coerce').fillna(0).round(0).astype(int)
        
        # فلتر الرينج على الأعمدة
        cols_to_show = ["Intervals"] + [c for c in final_intra.columns if start_date.strftime('%Y-%m-%d') <= c <= end_date.strftime('%Y-%m-%d')]
        
        st.session_state['df_intra'] = final_intra
        st.dataframe(final_intra[[c for c in final_intra.columns if c in cols_to_show]], use_container_width=True)
...
with tab3:
    ...
    if sched_file:
        ...
        # توليد التواريخ بناءً على الرينج المختار
...
with tab4:
    if 'df_intra' in st.session_state and 'df_cov' in st.session_state:
        ...
        if common_cols:
            ...
            st.warning("⚠️ No matching dates in the selected range.")
