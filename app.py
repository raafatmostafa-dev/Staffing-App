...
with tab2:
    ...
    if intra_file:
        ...
        # 2. تحويل الـ Intervals لنص صافي (HH:MM)
...
# ---------------------------------------------------------
# TAB 4: NET STAFFING (المقارنة وحل الـ NameError)
# ---------------------------------------------------------
with tab4:
    ...
    if 'df_intra' in st.session_state and 'df_cov' in st.session_state:
        ...
        # تعريف common_cols (حل الـ NameError)
        common_cols = [c for c in d_cov.columns if c in d_intra.columns]
        
        if common_cols:
            # توحيد الـ Index لضمان ظهور الداتا
            ...
            st.warning("⚠️ لا توجد تواريخ متطابقة بين الملفين.")
    else:
