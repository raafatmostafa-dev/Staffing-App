...
def check_auth():
    ...
    with col1:
        ...
        if st.button("Login"):
            # يوزر وباسورد ثابتين زي ما طلبت
...
if check_auth():
    ...
    with tab1:
        with st.sidebar:
            ...
            # رفع الملفات مع ميزة الحفظ التلقائي للثبات
        ...
        # التحقق من وجود ملف محفوظ (عشان الفيو يفضل ثابت)
    ...
    with tab2:
        if os.path.exists("intra_last.xlsx"):
            ...
            st.warning("يرجى رفع ملف 'Required.xlsx'.")
    ...
    with tab3:
        ...
        if os.path.exists("sched_last.xlsx") and lang:
            ...
            except: st.error(f"اللغة '{lang}' غير موجودة في ملف Schedules المحفوظ.")
